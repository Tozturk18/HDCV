...
static uint32_t checked_channel_index(const mxArray *array)
{
    double value;
    if (!mxIsDouble(array) || mxIsComplex(array) || mxGetNumberOfElements(array) != 1U) {
        mexErrMsgIdAndTxt("hdcv_mex:InvalidInput", "channelIndex must be a real double scalar.");
    }
    value = mxGetScalar(array);
    if (!mxIsFinite(value) || value < 0.0 || value > (double)UINT32_MAX || floor(value) != value) {
        mexErrMsgIdAndTxt("hdcv_mex:InvalidInput", "channelIndex must be a nonnegative integer.");
    }
...
)
{
    ...
    for (mwSize column = 0U; column < time_count; ++column) {
        uint32_t scan_index = nearest_scan_for_time(reader, times[column], active_channel);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    enum {
        ...
        FIELD_SAMPLES_PER_CHANNEL,
        FIELD_CURRENT_MATRIX_ROW_COUNT,
        FIELD_CHANNEL_INDEX,
        FIELD_CHANNEL_COUNT,
        FIELD_CHANNEL_NAME,
        FIELD_CVF_HZ
    ...
    if (nrhs < 2 || nrhs > 4) {
        mexErrMsgIdAndTxt(
            ...
            "Usage: out = hdcv_mex(filePath, signalTimes_s, backgroundTimes_s, channelIndex)");
    }
    if (!mxIsChar(prhs[0])) {
    ...
    if (nrhs >= 4) {
        channel_index = checked_channel_index(prhs[3]);
    }
    ...
    channel_count = reader.layout.channel_count == 0U ? 1U : reader.layout.channel_count;
    active_channel = channel_index % channel_count;
    ...
    if (!hdcv_reader_copy_voltage_for_channel(&reader, active_channel, voltage_float, points_per_scan)) {
    ...
    fill_cv_matrix(&reader, signal_times, signal_count, active_channel, channel_count, signal_matrix, signal_times_array, signal_scans_array);
    if (background_count > 0U) {
        fill_cv_matrix(&reader, background_times, background_count, active_channel, channel_count, background_matrix, background_times_array, background_scans_array);
    }
    ...
    mxSetFieldByNumber(out, 0, FIELD_SAMPLES_PER_CHANNEL, mxCreateDoubleScalar((double)reader.layout.samples_per_channel));
    mxSetFieldByNumber(out, 0, FIELD_CURRENT_MATRIX_ROW_COUNT, mxCreateDoubleScalar((double)reader.layout.current_matrix_row_count));
    mxSetFieldByNumber(out, 0, FIELD_CHANNEL_INDEX, mxCreateDoubleScalar((double)active_channel));
    mxSetFieldByNumber(out, 0, FIELD_CHANNEL_COUNT, mxCreateDoubleScalar((double)channel_count));
    mxSetFieldByNumber(out, 0, FIELD_CHANNEL_NAME, mxCreateString(hdcv_reader_channel_name(&reader, active_channel)));
